...
export async function register(
	...
	try {
		...
		const { firstName, lastName, email, password, passwordConfirm } = result.data;

		const response = await axios.post<{
			accessToken: string;
			user: User;
		}>(`${process.env.AUTH_SERVICE_HOST_URL}/api/v1/register`, {
			firstName,
			lastName,
			email,
			password,
		...
		const refreshCookie = parseCookie('refreshToken', response.headers['set-cookie']);
